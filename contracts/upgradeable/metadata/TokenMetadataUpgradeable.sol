...
abstract contract TokenMetadataUpgradeable is Initializable, ITokenMetadata {
    /// @custom:storage-location erc7201:forma.storage.TokenMetadataStorage
    struct TokenMetadataStorage {
        mapping(uint256 => bytes) _metadata;
    }

    // keccak256(abi.encode(uint256(keccak256("forma.storage.TokenMetadataStorage")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant TokenMetadataStorageLocation =
        0xb16b66e9489e60be4496a77b5e2be4c08e4e1277b5b80e25d6e8cd0d1ed16200;

    function _getTokenMetadataStorage() private pure returns (TokenMetadataStorage storage s) {
        assembly {
            s.slot := TokenMetadataStorageLocation
        }
    }
    ...
    function _exists(uint256 _tokenId) internal view virtual returns (bool) {
        TokenMetadataStorage storage s = _getTokenMetadataStorage();
        return s._metadata[_tokenId].length > 0;
    }
    ...
    function _getTokenMetadata(uint256 _tokenId) internal view returns (string memory) {
        TokenMetadataStorage storage s = _getTokenMetadataStorage();
        return CompressedJSON.unwrap(s._metadata[_tokenId]);
    }
    ...
    function _setTokenMetadataForced(uint256 _tokenId, string memory _metadata) internal virtual {
        TokenMetadataStorage storage s = _getTokenMetadataStorage();
        s._metadata[_tokenId] = CompressedJSON.wrap(_metadata);
    }
}
